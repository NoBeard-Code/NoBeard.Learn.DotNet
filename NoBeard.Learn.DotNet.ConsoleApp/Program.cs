namespace NoBeard.Learn.DotNet.ConsoleApp
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            IspisiClanove(new string[] { "Pero", "Perić", "Marko", "Marković" });
        ...
        static bool JeDjeljiv(int broj, int djelitelj)
        {
            if (djelitelj == 0)
            {
                return false;
        ...
        static void IspisiClanove(string[] niz)
        {
            Console.WriteLine($"Broj članova u nizu: {niz.Length}");

            string[] polovica = new string[niz.Length / 2];
            for (int i = 0; i < polovica.Length; i++)
            {
                polovica[i] = niz[i];
            }

            Console.WriteLine($"Broj članova u polovici: {polovica.Length}");

            if (polovica.Length > 0)
            {
                IspisiClanove(polovica);
            }
        }
    }
}